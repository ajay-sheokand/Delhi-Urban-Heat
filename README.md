...
- Toggleable **Photorealistic 3D** (beta): Google's Photorealistic 3D Tiles, streamed via Cesium ion (`deck.gl`'s `Tile3DLayer` + `CesiumIonLoader`, interleaved into the MapLibre map through `MapboxOverlay`). Genuinely higher detail than the OpenFreeMap buildings above — real photogrammetric mesh, not block massing — but it's a full opaque textured surface (ground, buildings, trees together), not a thin buildings-only layer, so enabling it visually replaces the heat/vegetation/land-cover map underneath rather than sitting alongside it. Uses a free-tier Cesium ion token (non-commercial use, capped at 1,000 root-tile requests/month account-wide — shared across all visitors, not per-visitor) — see [Known Limitations](#known-limitations).
...
- **District/ward/complementary-layer boundary lines are reordered to draw on top of Photorealistic 3D**, but this is stack-order reordering (`map.moveLayer`), not true surface draping — expect some z-fighting/flicker where 3D buildings poke through the flat boundary lines. Draping 2D data onto the mesh's actual geometry (so it follows roofs/ground correctly) would need deck.gl's `TerrainExtension`, which has a confirmed, unresolved upstream bug in interleaved mode for exactly this case ([visgl/deck.gl#7893](https://github.com/visgl/deck.gl/discussions/7893) — a maintainer reproduced it and it doesn't drape) — not attempted here for that reason. The LST/NDVI/land-cover raster layers and weather markers are unaffected by this reordering: rasters stay hidden under the opaque mesh (by design, matching the "explore mode" framing above), and weather markers were already visible on top regardless since they're DOM elements, not WebGL layers.
