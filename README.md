...
- Toggleable **Photorealistic 3D** (beta): Google's Photorealistic 3D Tiles, streamed via Cesium ion (`deck.gl`'s `Tile3DLayer` + `CesiumIonLoader`, interleaved into the MapLibre map through `MapboxOverlay`). Genuinely higher detail than the OpenFreeMap buildings above — real photogrammetric mesh, not block massing — and it's a full opaque textured surface (ground, buildings, trees together), not a thin buildings-only layer. The native MapLibre LST/NDVI/land-cover raster layers still stay hidden under that opaque mesh (same depth-buffer issue as the boundary layers below), so district/ward/complementary-layer boundaries **and** LST/NDVI/land-cover are redrawn as depth-disabled `deck.gl` layers (`GeoJsonLayer` for boundaries, `TileLayer`+`BitmapLayer` for the raster tiles) on top of the mesh, toggled by the same layer-panel checkboxes — see [Known Limitations](#known-limitations) for the depth-buffer workaround this relies on. Uses a free-tier Cesium ion token (non-commercial use, capped at 1,000 root-tile requests/month account-wide — shared across all visitors, not per-visitor) — see [Known Limitations](#known-limitations).
