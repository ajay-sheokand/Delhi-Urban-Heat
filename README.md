...
- Toggleable air-quality/pollution layers: Sentinel-5P (TROPOMI) NO₂, CO, and CH₄ (methane) tropospheric columns, same rolling-composite pattern as LST/NDVI (see [Air Quality & Pollution](#air-quality--pollution))
- District weather markers and heat alerts (OpenWeather, precomputed every 6h) — markers also show AQI/PM2.5 from OpenWeather's Air Pollution API when available
...
- Air quality & pollution: ward-level NO₂-vs-vulnerability-score correlation scatter, plus a district AQI bar chart from OpenWeather's live-refreshed readings (see [Air Quality & Pollution](#air-quality--pollution))
...
| [OpenWeather Air Pollution API](https://openweathermap.org/api/air-pollution) | District AQI (1-5 index), PM2.5/PM10/NO2/CO/O3/SO2 — same district points as weather | Point/model (SILAM) nowcast, refreshed every 6h by this pipeline | Requires API key (same key as Current Weather, server-side only) |
| [Sentinel-5P (TROPOMI) via Earth Engine](https://developers.google.com/earth-engine/datasets/catalog/sentinel-5p) — `COPERNICUS/S5P/OFFL/L3_NO2`, `L3_CO`, `L3_CH4` | Toggleable NO₂/CO/CH₄ map layers (both cities); ward-level NO₂ zonal mean + vulnerability-score correlation | ~1113m native pixel, 2-day revisit, composited over a rolling window (`AIR_QUALITY_COMPOSITE_DAYS`, default 30 days) | Free, [ESA/Copernicus](https://sentinels.copernicus.eu/), same GEE service account already used for LST/NDVI |
...
### Wind Field (interpolated, animated particle flow)

`weather.json`'s `wind_field` object (`build_wind_field()` in the precompute script) turns the ~9-11 real per-district wind readings above into a continuous field on the map (toggle: "🌬️ Wind Field", off by default), for a sense of general flow across the city rather than only at the exact district points. Method, spelled out because it's easy to overstate what a "field" implies:
...
Unlike the JJ-cluster/elderly-population layer above, this is **one shared implementation for both cities**, not a per-city substitution — NO₂-vs-vulnerability is the same hypothesis and the same field names everywhere, so there's no reason to fork it the way the complementary layer had to be.

**OpenWeather Air Pollution API** (`build_weather_dataset()`, same function that builds `weather.json`): a second per-district API call (`/data/2.5/air_pollution`, same key/account as the existing Current Weather call, its own inner try/except so a failed pollution fetch never drops that district's temperature/wind data too) adds `aqi` (OpenWeather's own 1-5 index), `aqi_label` (`aqi_label()` — real OpenWeather-documented thresholds: 1=Good, 2=Fair, 3=Moderate, 4=Poor, 5=Very Poor, e.g. "Moderate" = NO2 100-200 / PM2.5 30-55 µg/m³), and ground-level `pm2_5`/`pm10`/`no2_ugm3`/`co_ugm3`/`o3_ugm3`/`so2_ugm3` (µg/m³) to each district record already in `weather.json`. This is a **model estimate (SILAM, the Finnish Meteorological Institute's atmospheric model)**, not a raw CPCB/official-station reading — same "modeled, not measured" caveat this project already applies to WorldPop.

**Sentinel-5P (TROPOMI) map layers** (`build_map_layers_dataset()`): NO₂ (`COPERNICUS/S5P/OFFL/L3_NO2`, band `tropospheric_NO2_column_number_density`), CO (`COPERNICUS/S5P/OFFL/L3_CO`, band `CO_column_number_density`), and CH₄ (`COPERNICUS/S5P/OFFL/L3_CH4`, band `CH4_column_volume_mixing_ratio_dry_air`) are each composited with `.mean()` over a rolling `AIR_QUALITY_COMPOSITE_DAYS`-day window (default 30 days — a separate, independent knob from `MAP_COMPOSITE_DAYS`'s 45-day Landsat window, since S5P's daily coverage is sparser/cloudier), clipped to the region, and rendered via the same dynamic-min/max-with-fallback `getMapId()` pattern already used for LST. **These are tropospheric column densities (mol/m² for NO2/CO, ppb for CH4) — the total gas in a vertical slice of atmosphere — not ground-level concentrations people breathe.** This is a genuinely different physical quantity from the OpenWeather numbers above, not just a different precision, and the two should never be read as interchangeable "pollution" numbers.
...
Split into two independently-refreshed pieces (see the second postmortem below for why): `build_building_footprint_layer()` in `map_layers.json`'s `layers.buildings` (a cheap `ee.Image().paint()` + `getMapId()` tile layer, regenerated every 6-hourly run like every other map layer), and `build_building_stats_dataset()` in `building_stats.json` (the expensive real count, only on the weekly cadence). Both query Google Open Buildings v3 (`GOOGLE/Research/open-buildings/v3/polygons`), an AI building-footprint dataset detected from satellite imagery, filtered to the city's real boundary. Delhi's real count, confirmed by direct timing against the live Earth Engine service: **2,432,335 buildings across 1,487 km²** (~1,635/km², ~154s for the `FeatureCollection.size()` call).

`FeatureCollection.size()` over a whole city is a genuinely expensive server-side vector scan (confirmed directly: a ~120 km² sub-area alone took ~33s, and cost did not scale down proportionally for smaller areas — there's a large fixed per-call cost), so the count stays on the same weekly cadence as `historical_trends.json` (`should_run_weekly_job()`) — reasonable since Open Buildings v3 is a static satellite-derived snapshot, not something that changes hour to hour. The footprint *tile layer*, in contrast, is cheap (no `.getInfo()` call - `getMapId()` doesn't force any server-side computation) and needs to be regenerated far more often than weekly - see the second postmortem below.

> [!IMPORTANT]
> **Delhi-only, and this is a real coverage limit, not an oversight:** Open Buildings v3's stated coverage is the Global South (Africa, South/Southeast Asia, Latin America, Caribbean, Middle East) — confirmed directly by querying Münster's bbox and getting back exactly zero features. There is no equivalent building-footprint layer for Münster on this site; the toggle row is hidden entirely there (`row-buildings` stays `display:none` when `map_layers.json` has no `buildings` key) rather than shown empty or backfilled with a different, non-comparable source.
>
...
> [!WARNING]
> **Postmortem — this file was silently disappearing from the live site every ~6 hours, found the same day it shipped.** Reported as "the Photorealistic 3D wrap functions aren't working for buildings"; the actual bug had nothing to do with deck.gl.
>
> **Root cause:** the GitHub Actions workflow's "Seed backend-data from previous publish" step (`.github/workflows/precompute-backend-data.yml`) carries forward a *hardcoded list* of filenames from the previous `gh-pages` publish, for exactly the files `should_run_weekly_job()` skips regenerating on an ordinary run. `building_stats.json` was added to `main()`'s weekly-only branch but never added to that seed list. Consequence: the first forced/manual run correctly published it — but the very next ordinary 6-hourly scheduled run (which doesn't recompute it, and now had nothing to carry forward either) republished `gh-pages` *without it*. Confirmed directly: the file 404'd live within an hour of a successful forced run, immediately after the next regular schedule fired. `historical_trends.json` has the identical weekly-only pattern and was fine, only because it was already in the seed list from before this feature existed — this wasn't a new kind of mistake, just a missed line for a new file following an existing one. Fix: added `building_stats.json` to the seed list.
>
> **A second, real but narrower bug found and fixed at the same time, unrelated to the above:** `loadBuildingStats()` (and the district/ward/complementary-layer boundary loaders) fetch their data independently of `map_layers.json`, each on their own network round trip. If a user switches Photorealistic 3D on before one of those resolves, `buildRasterOverlayLayers()`/`buildBoundaryOverlayLayers()` runs once with that layer's cache still empty and silently omits it from the mesh — and unlike every checkbox's own `change` handler (which already calls `syncBoundaryOverlayLayers()` on every toggle), nothing was re-triggering that sync once the late-arriving fetch finished. Each loader now calls `syncBoundaryOverlayLayers()` itself once, right after wiring up, closing that gap. This is real and worth having fixed, but **it was not the actual cause of the symptom that was reported** — the 404 above was — which is worth stating plainly rather than letting a plausible-sounding secondary fix stand in as the explanation for what was actually observed.
>
> **A third bug, found days later while diagnosing a completely different problem (why Photorealistic 3D overlays weren't truly draping onto the mesh — see the Photorealistic 3D bullets under [Known Limitations](#known-limitations)):** the building footprint *tile* was generated only inside `build_building_stats_dataset()`, meaning it only got a fresh Earth Engine `getMapId()` token on the same weekly cadence as the count. Confirmed directly against production: a mapid generated 2026-08-24 was returning HTTP 401 on every tile request by 2026-08-26 — Earth Engine's tile-serving tokens expire well within a week, so the footprint layer was silently broken for most of every week, this whole time. Worse, this wasn't limited to whoever had the Buildings toggle checked: deck.gl's `Tile3DLayer`/`TileLayer` background-fetches tiles for layers already registered in `rasterLayerCache` regardless of their `visible` state, confirmed by capturing 88 failed requests against the stale buildings mapid during a test that never touched the buildings checkbox at all. Fix: split the tile generation out into `build_building_footprint_layer()`, called from `build_map_layers_dataset()` on every 6-hourly run (cheap - no `.getInfo()` needed) so it always has a fresh token, while the expensive count stays weekly in `build_building_stats_dataset()`. The frontend now gets the tile from `map_layers.json.layers.buildings` (`wireBuildingFootprintsLayer()`) and the count from `building_stats.json` (`loadBuildingCount()`) as two independent, gracefully-degrading pieces — this also incidentally closes the same load-order-race class as the second postmortem above, since the tile is now wired synchronously inside `loadMapLayers()` instead of a separate late-arriving fetch.
...
- **District/ward/complementary-layer boundaries, and LST/NDVI/land-cover/pollutants/flood-risk/buildings, render on top of AND genuinely conform to Photorealistic 3D's real mesh geometry** as deck.gl layers (`GeoJsonLayer` for boundaries, `TileLayer`+`BitmapLayer` for the raster tiles), not the native MapLibre fill/line/raster layers (which stay hidden underneath, invisible). This went through two real implementations, both honestly documented here since the first one shipped and worked for weeks before the second was found:
  - **Approach 1 (shipped first, since replaced):** `{ depthCompare: "always", depthWriteEnabled: false }` on each overlay layer, in deck.gl's *interleaved* `MapboxOverlay` mode. Two alternatives were tried and ruled out first: draping via deck.gl's `TerrainExtension` has a confirmed, unresolved upstream bug in interleaved mode specifically ([visgl/deck.gl#7893](https://github.com/visgl/deck.gl/discussions/7893) — a maintainer reproduced it and it doesn't drape); reordering the native MapLibre layers to draw after the mesh (`map.moveLayer`) does nothing, since once the interleaved 3D layer writes real depth values, MapLibre's own 2D layers get depth-tested against it regardless of paint order (verified directly with a bright test line that stayed invisible). `depthCompare: "always"` bypasses the depth test entirely, which draws the data on top - but only in the sense of "always visible above," not real draping: it's a flat plane at a fixed position, with no relationship to the mesh's actual rooftop/terrain geometry underneath. (The accompanying `depthWriteEnabled: false` was needed too, for a separate reason: `depthCompare: "always"` alone only disables the *test*, not deck.gl's default `depthWriteEnabled: true` - these flat layers were still writing meaningless depth values into the shared buffer, which caused visible flicker while rotating the camera as the mesh's own internal depth test compared its real depth against those stale writes frame to frame.)
  - **Approach 2 (current):** real terrain-conforming draping, confirmed directly with an isolated test page against the live tileset before touching production code. The `#7893` interleaved-mode bug does **not** reproduce in deck.gl's default *non-interleaved* overlay mode (its own separate canvas, not sharing MapLibre's WebGL context) - screenshotted proof: a test boundary polygon's edge sits exactly along real ground/building contours instead of floating as a disconnected rectangle. `MapboxOverlay` now uses `interleaved: false`; the `Tile3DLayer` gets `operation: "terrain+draw"` (serves as both the rendered mesh and the terrain surface other layers drape onto); every overlay layer gets `extensions: [new deck._TerrainExtension()]` + `terrainDrawMode: "drape"` instead of the depth-test bypass. Giving up MapLibre interleaving costs nothing here, since MapLibre's own 2D layers are already irrelevant underneath the opaque mesh whenever Photorealistic 3D is active.

  Both approaches share the same raster-tile-loading fix: `TileLayer`'s default tile-loading path decodes fetched tiles via a loaders.gl `ImageLoader`, but this page only loads `deck.gl` core and `@loaders.gl/3d-tiles` (for the mesh itself) — `@loaders.gl/images` is never on the page, and `deck.gl`'s own UMD bundle doesn't bundle it either (confirmed absent from the built bundle) — so the default path fetches tile bytes but has nothing to decode them into a usable image, and nothing draws. Fixed with a custom `getTileData` that fetches and decodes via the browser's native `createImageBitmap` instead, sidestepping loaders.gl entirely for this one path. The ward boundary line is deliberately drawn thicker here (`lineWidthUnits: "pixels"`, 2.5px fixed) than its flat-map counterpart, for legibility against the busier photorealistic imagery. Weather markers were already visible on top regardless, since they're DOM elements, not WebGL layers.
...
- **OpenWeather's AQI/pollutant numbers are a model estimate (SILAM), not a raw official-station reading** — same caveat as WorldPop's "modeled, not measured" population figures. Cross-checking against India's CPCB network or Germany's official air-quality stations directly was out of scope for this pass.
- **Sentinel-5P NO₂/CO/CH₄ are tropospheric/total column densities (mol/m² or ppb), not ground-level concentrations** — a genuinely different physical quantity from the OpenWeather AQI/PM numbers on the same site, not just a different precision. Don't read them as two measurements of the same thing.
- **Sentinel-5P's ~1.1km native pixel is far coarser than Landsat's 100m thermal band**, and cloud/aerosol quality flags mean some readings within the 30-day composite window are noisier than others (not separately filtered by quality flag in this pass). The CH₄ collection also has a real gap in Google's archive (2022-07-26 to 2022-08-31) — only relevant if a future `historical_trends.json`-style long-window view is ever added for pollution, which it isn't here.
- **CH₄'s toggle can go missing for a city with no valid retrievals in the current window** — TROPOMI's CH₄ retrieval is SWIR-based and needs clear-sky scenes, and published research confirms monsoon cloud cover creates real coverage gaps in South Asian S5P CH₄ data ([ACP 2026](https://acp.copernicus.org/articles/26/9757/2026/), [Springer 2024](https://link.springer.com/article/10.1007/s11356-024-35755-9)). `pollutant_layer()` in `build_map_layers_dataset()` retries once with a wider 90-day composite when the primary window comes back empty; if that's still empty (observed for Delhi during its 2026 monsoon), the layer key is omitted from `map_layers.json` entirely rather than publishing a blank tile under a fabricated legend range, and the frontend disables that toggle with an explanatory tooltip instead of adding a source/layer that can't load. NO₂ and CO haven't needed the retry path so far.
- **Only NO₂ gets a ward-level vulnerability-score correlation; CO and CH₄ are map-only layers**, not run through a second/third near-duplicate correlation section — see [Air Quality & Pollution](#air-quality--pollution) for why.
...
1. Add GitHub Actions secrets in your repo:
   ...
   - `MAP_COMPOSITE_DAYS`, `ANALYTICS_AIR_TEMP_DAYS`, and `AIR_QUALITY_COMPOSITE_DAYS` are plain env vars with defaults (`45`, `90`, and `30` respectively) inside `scripts/precompute_timeseries_backend.py` — edit the script if you want different rolling windows; not required secrets.
...
August 26, 2026 (Photorealistic 3D overlays now genuinely drape onto the mesh's real geometry — building rooftops, terrain contours — instead of floating as a flat plane above it; switched from `depthCompare:"always"` to non-interleaved `MapboxOverlay` + deck.gl's `TerrainExtension` in real `"drape"` mode, confirmed with a screenshotted isolated test before touching production. While chasing that, found and fixed a real bug it exposed: the building-footprint tile's Earth Engine token was only refreshed on the weekly cadence and was expiring mid-week, breaking the layer for most of every week regardless of whether the toggle was even checked — the footprint tile is now regenerated every 6-hourly run like every other map layer, independent of the expensive weekly building count. Full write-up in the Photorealistic 3D bullets under [Known Limitations](#known-limitations) and the postmortems under [Building Footprints & Count](#building-footprints--count-delhi-only). August 19, 2026: fixed `building_stats.json` being silently dropped from every non-weekly precompute run — a missing filename in the workflow's seed-forward list. Added Flood Risk — an SRTM elevation-percentile relative-risk map layer — and a real Delhi building count/footprint layer from Google Open Buildings v3; see [Flood Risk](#flood-risk-elevation-percentile-proxy) and [Building Footprints & Count](#building-footprints--count-delhi-only). Map page rebuilt around a persistent sidebar with its own visual identity — see [Visual Design](#visual-design) — replacing the earlier floating-panel layout; `analytics.html`/`roadmap.html`/`about.html` re-themed via the same shared design tokens)
