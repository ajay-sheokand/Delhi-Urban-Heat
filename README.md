...
- **Photorealistic 3D is a free-tier Cesium ion feature, not a permanent guarantee.** It's capped at 1,000 root-tile requests/month **account-wide** (shared across every visitor to the site, not per-visitor) — once exhausted for the month, the layer will fail to load until the quota resets, rather than degrading gracefully to a lower-detail view. The free tier is also restricted to non-commercial/individual use per Cesium ion's terms. Unlike the OpenFreeMap buildings layer, this is a mesh of the entire visible surface (rendered translucent, `opacity: 0.6`, rather than fully solid — see above), so enabling it still substantially replaces the map's heat/vegetation/land-cover visualization underneath rather than cleanly layering on top of it — it's closer to an "explore" mode than an additional data layer, even with the mesh no longer fully opaque.
